...
Copyright 2000, 2011 Advanced Distributed Learning (ADL). Some Rights Reserved.
*****************************************************************************
...
namespace ADL.SCORM12 {
    export var debug = true;  // set this to false to turn debugging off

    export var output = window.console; // output can be set to any object that has a log(string) function
    // such as: var output = { log: function(str){alert(str);} };

    // Define exception/error codes
    export var _NoError = { "code": "0", "string": "No Error", "diagnostic": "No Error" };
    export var _GeneralException = { "code": "101", "string": "General Exception", "diagnostic": "General Exception" };
    ...
    **
    *******************************************************************************/
    export function doLMSInitialize(): string {
        ...
        if (result.toString() != "true") {
            ...
            message("LMSInitialize failed with: " + err.string + " error code: " + err.code + " Message: " + err.diagnostic);
            if (err.code == "101") {
                initialized = true;
            }
        }
    ...
    *******************************************************************************/
    function getAPI() {
        ...
        try {
            ...
            if ((theAPI == null) && (window.opener.top.opener != null) && (typeof (window.opener.top.opener) != "undefined")) {
                theAPI = findAPI(window.opener.top.opener);
            }
        }
        catch (e) {
            theAPI = null;
